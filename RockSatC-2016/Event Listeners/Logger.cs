...
namespace RockSatC_2016.Event_Listeners {
    public class Logger  {
        ...
        public Logger(string comPort, int baud, int maxBuffer = 512) {
            ...
            var unused = new byte[] {};
            _workItem = new ThreadPool.WorkItem(LogWorker, ref unused, persistent: true);
        ...
        struct QueuePacket {
            ...
            public QueuePacket(EventType eventName, IEventData eventData, ref byte[] arrayData) {
                Name = eventName;
                EventData = eventData;
                ArrayData = arrayData;
            }
        ...
        private void OnDataFound(EventType eventName, IEventData trigger, ref byte[] arrayData) {
            ...
            _pendingData.Enqueue(new QueuePacket(eventName, trigger, ref arrayData));
        }
